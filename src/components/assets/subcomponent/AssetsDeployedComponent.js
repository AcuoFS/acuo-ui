    ...
    return _.reduce(rawAPI,
      (accumulatedResults, agreementObj, id) => {
        ...
        if (searchInAssets) return _.concat(accumulatedResults, searchInAssets)
          ...
          if (searchRegAgrCpty) return _.concat(accumulatedResults, searchRegAgrCpty)
...
const AssetsDeployedComponent = (props) => {
  ...
  let cellWidth = (ExpandedSideways ? {
    category: [26, 37, 37],
    data: [17, 10, 18, 18, 7, 7, 10, 13]
  } : {category: [26, 37, 37], data: [20, 16, 27, 27, 10]})
      ...
      return _.map(filteredContent, (row) => {
        return {
          ...
          RowContent: _.map(row.data, (block) => {
            return {
              assetID: block.id,
              assetInfo: [block.asset, block.quantity, block.adjValue, block.value, block.rating, block.haircut, block.maturityDate, block.isin]
            }
          }),
      ...
      return _.map(filteredContent, (row) => {
        return {
          ...
          RowContent: _.map(row.data, (block) => {
            return {
              assetID: block.id,
              assetInfo: [block.asset, block.quantity, block.adjValue, block.value, block.haircut]
            }
          }),
          PledgeContent: ["Pledge", " ", row.pledge.adjValue, row.pledge.value, " "],
          ExcessContent: ["Excess", " ", row.excess.adjValue, row.excess.value, " "],
    ...
    <div className={ExpandedVertically ? (styles.assetsPanelFrameExpandedVertically) : (styles.assetsPanelFrame)}>
      <div className={styles.assetsPanelHeader}>
        ...
        <input className={styles.assetsPanelHeaderInput}
               ...
               placeholder={"Search"}
               onChange={(e) => {
                 actions.DeployedPanel_SearchText(e.target.value)
               }}/>
        <img className={styles.assetsPanelHeaderSideExpandBtn}
             ...
             onClick={() => {
               actions.DeployedPanel_ToggleSideExpand(!ExpandedSideways)
             }}/>
      </div>
      <PanelWindow>
      ...
      <div className={styles.panelResizeHandle}
           onClick={() => {
             actions.DeployedPanel_ToggleVerticalExpand(!ExpandedVertically)
           }}>
        &#9650;  &#9660;
      </div>
    </div>
